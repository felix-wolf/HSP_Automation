...
def send_notification(message):
    ...
    if not telegram_id:
        return
    id = int(os.environ.get('TELEGRAM_ID'))
    header={
        "chat_id": id,
        "text":message,
    }
    response = requests.post(f"https://api.telegram.org/bot{os.environ.get('TELEGRAM_TOKEN')}/sendMessage", json=header)
...
if __name__ == "__main__":
    if len(sys.argv) <= 1:
        ...
        if eval(os.environ.get('CALID')).get(user, False):
            try:
